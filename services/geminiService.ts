...
export const extractLabData = async (text: string, videoData?: { data: string, mimeType: string }): Promise<LabResults> => {
  ...
  const prompt = `You are an expert medical data extractor specialized in parsing Ukrainian laboratory test results.

${hasVideo ? "Analyze the provided VIDEO of medical reports. Watch the video carefully to identify parameter names and their values. The video may pan over a long document." : "Analyze the unstructured text provided below."}
